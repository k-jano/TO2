    		...
    		try {
    	        ...
    	        for(String photoUrl : photoUrls) {
    	        	...
    	        	observer.onNext(getPhoto(photoUrl));
    	        		...
    	        		log.log(Level.WARNING, "Could not get photo", e);
    	        	}
    	        }
    	        observer.onComplete();
    		}
    		catch (IOException e) {
                log.log(Level.WARNING, "Could not download a photo in searchingForPhotos", e);
            }
    	});
    }

    public Observable<Photo> searchForMultiplePhotos(List<String> searchQueries) throws IOException { 	
    	List<Observable<Photo>> listO = new ArrayList<>();

    	for(String searchQuery : searchQueries) {
    		listO.add(searchForPhotos(searchQuery).subscribeOn(Schedulers.io()));

    	}
    	Observable<Photo> o_final = Observable.merge(listO);
    	return o_final;
}
